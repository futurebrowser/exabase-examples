...
import { workspaceKeys } from "@/lib/api/query-keys";
...
function documentsNeedPoll(
  data: { documents: { stateProcessing: string | null }[] } | undefined,
): boolean {
...
export function WorkspaceApp({
  ...
  const documentsQuery = useQuery({
    ...
    refetchInterval: (query) =>
      documentsNeedPoll(query.state.data) ? 4000 : false,
  });

  const flashcardsQuery = useQuery({
    queryKey: workspaceKeys.flashcards(workspaceId),
    queryFn: () => getFlashcards(workspaceId),
    refetchInterval: 10000,
  });

  const uploadMutation = useMutation({
    mutationFn: (files: File[]) =>
      Promise.all(files.map((file) => postUpload(workspaceId, file))),
    onSuccess: (data) => {
      setNotice(
        data.length === 1
  ...
  const generateMutation = useMutation({
    mutationFn: () => postGenerateFlashcards(workspaceId, { topic, count }),
    onSuccess: (data) => {
      if (data.thinContext) {
        setNotice(
          `Created ${data.created} cards. Context was limited — try again after PDF processing completes.`,
        );
      } else {
        setNotice(`Created ${data.created} cards.`);
      }
      void queryClient.invalidateQueries({
        queryKey: workspaceKeys.flashcards(workspaceId),
      });
  ...
  const deleteAllMutation = useMutation({
    mutationFn: () => deleteAllFlashcards(workspaceId),
    onSuccess: (data) => {
      setNotice(`Deleted ${data.deleted} flashcard(s).`);
      void queryClient.invalidateQueries({
        queryKey: workspaceKeys.flashcards(workspaceId),
      });
    },
      ...
      <Card>
        <CardHeader className="flex min-w-0 flex-row flex-wrap items-start justify-between gap-2">
          <div className="min-w-0 flex-1 space-y-1">
            ...
            <CardDescription>
              Link-only access (demo). Anyone with this URL can use this
              workspace.
            </CardDescription>
          </div>
          <Button
            ...
            <ul className="min-w-0 divide-y divide-border rounded-none border border-border">
              {documents.map((d) => {
                ...
                return (
                  <li key={d.id}>
                    <Link
                      href={`/api/w/${workspaceId}/documents/${d.id}`}
                      className="flex flex-wrap items-center justify-between gap-2 px-3 py-2 transition-colors hover:bg-muted/40"
                    >
        ...
        <CardHeader className="flex min-w-0 flex-row flex-wrap items-start justify-between gap-2 space-y-0">
          ...
          <Button
            ...
            onClick={() => {
              if (
                !confirm(
                  "Delete all flashcards in this workspace? This cannot be undone.",
                )
              ) {
                return;
              }
