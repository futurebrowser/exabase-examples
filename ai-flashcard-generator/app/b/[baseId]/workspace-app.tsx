...
import { baseKeys } from "@/lib/api/query-keys";
...
export function WorkspaceApp({
  ...
  const shareUrl = appOrigin ? `${appOrigin}/b/${baseId}` : `/b/${baseId}`;

  const documentsQuery = useQuery({
    queryKey: baseKeys.documents(baseId),
    queryFn: () => getDocuments(baseId),
    refetchInterval: (query) =>
      documentsNeedPoll(query.state.data) ? 4000 : false,
  });

  const flashcardsQuery = useQuery({
    queryKey: baseKeys.flashcards(baseId),
    queryFn: () => getFlashcards(baseId),
    refetchInterval: 10000,
  });

  const uploadMutation = useMutation({
    mutationFn: (files: File[]) =>
      Promise.all(files.map((file) => postUpload(baseId, file))),
    onSuccess: (data) => {
      setNotice(
        data.length === 1
  ...
  const generateMutation = useMutation({
    mutationFn: () => postGenerateFlashcards(baseId, { topic, count }),
    onSuccess: (data) => {
      if (data.thinContext) {
        setNotice(
  ...
  const deleteAllMutation = useMutation({
    mutationFn: () => deleteAllFlashcards(baseId),
    onSuccess: (data) => {
    ...
    <div className="mx-auto flex w-full min-w-0 max-w-xl flex-col gap-6 px-4 py-16">
      ...
      <Card>
        ...
        <CardContent className="min-w-0">
          <Badge variant="outline">{shareUrl}</Badge>
        </CardContent>
      </Card>
            ...
            <ul className="min-w-0 divide-y divide-border rounded-none border border-border">
              {documents.map((d) => {
                ...
                return (
                  <li key={d.id}>
                    <Link
                      href={`/api/b/${baseId}/documents/${d.id}`}
