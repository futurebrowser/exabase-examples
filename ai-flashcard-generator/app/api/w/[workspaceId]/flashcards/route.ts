...
async function getFlashcardMemoryHits(workspaceId: string) {
...
export async function GET(_request: Request, context: RouteCtx) {
  ...
  const hits = await getFlashcardMemoryHits(workspaceId);
  ...
  const body = flashcardsResponseSchema.parse({ flashcards });
...
export async function DELETE(_request: Request, context: RouteCtx) {
  ...
  for (const id of ids) {
    await api.memories.delete({
      workspaceId,
      memoryId: id,
    } as Parameters<typeof api.memories.delete>[0]);
  }

  const body = deleteAllFlashcardsResponseSchema.parse({
    deleted: ids.length,
  });
  return NextResponse.json(body);
}
